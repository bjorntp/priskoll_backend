...
BEGIN
    ...
    INSERT INTO product_prices ("productNumber", "oldPrice", "newPrice", "dateOfChange")
    VALUES (OLD."productNumber", OLD.price, NEW.price, NOW());

    -- Update the products table with old price + timestamp
    UPDATE products
    SET "oldPrice" = OLD.price,
        "dateOfPriceChange" = NOW()
    WHERE "productNumber" = OLD."productNumber";
